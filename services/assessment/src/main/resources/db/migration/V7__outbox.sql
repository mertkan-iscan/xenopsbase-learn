-- The transactional outbox and the consumed-message ledger (T-9.8), arriving in
-- this module with grading (T-6.7).
--
-- V1 said assessment "publishes nothing and consumes nothing yet, and
-- `enabled: false` is what keeps the relay, its scheduled task and the broker
-- connection out of the process entirely. Grading (T-6.7) is what turns this on,
-- because a graded attempt is the first thing here another module has to learn
-- about." This is that.
--
-- The DDL below is catalog's, unchanged. Flyway locations are per module, so it
-- is repeated per service while the behaviour is not -- which is the right way
-- round, and the paragraph after this one is the original's explanation of why.
--
