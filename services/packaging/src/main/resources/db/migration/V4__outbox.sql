-- V4 — the transactional outbox and the consumed-message ledger (T-9.8), now
-- that this service has something to announce: a package said a learner
-- finished (T-4.4).
--
...
--
-- That window is not hypothetical here. The row this announces is written by a
-- learner's browser relaying what a package said, on a connection that drops:
-- the save committing and the announcement not is exactly the failure that
-- leaves somebody with a finished course and a gate that will not open.

-- The column is `topic` rather than `subject` for the reason streaming's copy
-- states: the ADR-0104 tripwire in each service's app test matches any column
-- shaped like a stored `sub`, and `subject` is exactly that shape. Moving out of
-- the way of a false positive is better than widening the rule that catches the
-- real thing.
CREATE TABLE outbox (
    id             uuid         PRIMARY KEY,
