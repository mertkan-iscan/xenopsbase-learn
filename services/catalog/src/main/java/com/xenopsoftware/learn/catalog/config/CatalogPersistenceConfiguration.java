 ...
 *
 * ADR-0109 puts {@code identity}, {@code catalog} and {@code assessment} in one deployable while
 ...
 * silently bind to <em>identity's</em>. It would not fail. The catalog repository inside it would
 ...
 * call. {@code CatalogTransactionsAreQualifiedTest} moves it earlier still, to the build.
 *
 ...
 * is the only thing resolved at runtime: a {@code catalogDataSource} bean if one exists (the
...
@EnableJpaRepositories(
    basePackages = "com.xenopsoftware.learn.catalog",
...
public class CatalogPersistenceConfiguration {
    ...
    static final String UNIT = "catalog";

    @Bean
    LocalContainerEntityManagerFactoryBean catalogEntityManagerFactory(
            EntityManagerFactoryBuilder builder,
            @Qualifier("catalogDataSource") ObjectProvider<DataSource> catalogDataSource,
            ObjectProvider<DataSource> theOnlyDataSource) {
        return builder
            .dataSource(catalogDataSource.getIfAvailable(theOnlyDataSource::getObject))
            ...
            .packages("com.xenopsoftware.learn.catalog")
