...
public class StreamingMessagingConfiguration {

    /** Tags this module's meters and names its durable consumers. Unique per module, by rule. */
    private static final String MODULE = "streaming";

    @Bean
    Outbox streamingOutbox(@Qualifier("streamingDataSource") ObjectProvider<DataSource> named,
    ...
    @Bean
    OutboxRelay streamingOutboxRelay(@Qualifier("streamingDataSource") ObjectProvider<DataSource> named,
    ...
    @Bean
    OutboxMetrics streamingOutboxMetrics(
            @Qualifier("streamingOutboxRelay") OutboxRelay relay, MeterRegistry meters) {
        return ModuleMessaging.metrics(relay, meters, MODULE);
    }

    @Bean
    ConsumedMessages streamingConsumedMessages(@Qualifier("streamingDataSource") ObjectProvider<DataSource> named,
            ObjectProvider<DataSource> theOnlyDataSource) {
        return ModuleMessaging.consumed(named.getIfAvailable(theOnlyDataSource::getObject));
    }

    @Bean
    NatsSubscriber streamingNatsSubscriber(ObjectProvider<Connection> connection,
            List<MessageHandler> handlers,
            @Qualifier("streamingConsumedMessages") ConsumedMessages consumed,
            @Qualifier("streamingTransactionManager") ObjectProvider<PlatformTransactionManager> named,
